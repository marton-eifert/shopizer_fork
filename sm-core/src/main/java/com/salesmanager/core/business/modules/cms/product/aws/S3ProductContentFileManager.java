  ...
  public List<OutputContentFile> getImages(String merchantStoreCode,
    ...
    try {
      ...
      ListObjectsV2Request listObjectsRequest = new ListObjectsV2Request()
        ...
        /**********************************
         * CAST-Finding START #1 (2024-02-02 12:30:41.601250):
      ...
      // QECI Fix: Move instantiation outside loop
      ...
      for (S3ObjectSummary os : objects) {
        ...
        if (!StringUtils.isBlank(mimetype)) {
          ...
          byte[] byteArray = IOUtils.toByteArray(o.getObjectContent());
            ...
            /**********************************
             * CAST-Finding START #2 (2024-02-02 12:30:41.601250):
             * TITLE: Avoid instantiations inside loops
             * DESCRIPTION: Object instantiation uses memory allocation, that is a greedy operation. Doing an instantiation at each iteration could really hamper the performances and increase resource usage.  If the instantiated object is local to the loop, there is absolutely no need to instantiate it at each iteration : create it once outside the loop, and just change its value at each iteration. If the object is immutable, create if possible a mutable class. If the aim is to create a consolidated data structure, then, unless the need is to release the data case by case, it could be better to make a single global allocation outside the loop, and fill it with data inside the loop.
             * STATUS: WITHDRAWN
             * CAST-Finding END #2
          ...
          baos.write(byteArray, 0, byteArray.length);
            ...
            /**********************************
             * CAST-Finding START #3 (2024-02-02 12:30:41.601250):
             * TITLE: Avoid instantiations inside loops
             * DESCRIPTION: Object instantiation uses memory allocation, that is a greedy operation. Doing an instantiation at each iteration could really hamper the performances and increase resource usage.  If the instantiated object is local to the loop, there is absolutely no need to instantiate it at each iteration : create it once outside the loop, and just change its value at each iteration. If the object is immutable, create if possible a mutable class. If the aim is to create a consolidated data structure, then, unless the need is to release the data case by case, it could be better to make a single global allocation outside the loop, and fill it with data inside the loop.
             * STATUS: WITHDRAWN
             * CAST-Finding END #3
